...
output$DT_table <- renderDataTable({
  ...
  summarized_data() %>% 
    select(Alpha2code=country_taq,Country = country, Allele, 
           ...
           `Number of datapoints` =Number_of_studies,
    ...
    datatable(.,extensions = 'Buttons',
              ...
              options =  list(pageLength = 25, 
                              ...
                              columnDefs = list(list(width = '200px', targets = 6)), # 0 based column index
                              dom = 'Bfrtip',
                              buttons = c('copy', 'csv', 'excel', 'pdf')))})
  ...
  summarized_data <- reactive({
    ...
    req(input$allele)
     ...
     dataset() %>% 
      ...
      summarise(Frequency = round(median(Freq, na.rm = T),3),
                Number_of_studies = n(),
                Frequency_CI =wilcox.test(Freq, conf.int = T, exact = F) %>% 
                  tidy %>% 
                  select(starts_with("conf")) %>%
                  mutate_all(round, 2) %>% 
                  paste(., collapse = "-") %>% 
                  paste0("CI=(", .,")"),
                Weighted = matrixStats::weightedMedian(Freq, size, na.rm = T) %>% round(., 3),
                Weighted_MAD = matrixStats::weightedMad(Freq, size, na.rm = T, constant = 1) %>% round(., 3),
   ...
   summarized_data() %>% 
      ...
      gvisGeoChart(., locationvar='country_taq', colorvar='Frequency', hovervar = "country",
                   options=list(region= df_regions %>% filter(region == input$region) %>% pull(code),
                                ...
                                colorAxis=paste0("{colors:['",input$col_min, "','", input$col_mid1,"','",input$col_mid2,"','", input$col_max,"']}")))
  })
